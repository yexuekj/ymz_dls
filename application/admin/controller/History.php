...
class History extends Base
{
    ...
    {
        if($this->role_id == 1){
            $host = Db::connect(\config('data_base_3'))->table('ipdata')->where('status',1)->select();
        }else{
            $ipdata = Db::table('user_host')->select();
            $ipdata = array_column($ipdata,'ipdata_id','ipdata_id');
            $host = Db::connect(\config('data_base_3'))->table('ipdata')->where('status',1)->whereIn('id',$ipdata)->select();
        }
